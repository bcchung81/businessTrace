...
운영자가 3분 안에 훑는 화면이므로 **요약문이 헤드라인이다.** 밴드(`--band`)에 H1 과 요약문을 크게 두고, 그 밑을 primary 띠가 신선도 항목으로 잇는다. 절은 4px 굵은 괘선과 큰 번호로 나뉜다. 카드 상자·하드 그림자·크림지·코믹북 리본은 모두 걷어낸다. 서체는 Pretendard 본문 + **Gothic A1 900** 디스플레이(H1·절 번호·절 제목·리본 수치).

트레이드오프(기록): 다크에서 밴드와 배경의 구분이 흐려진다 — 다크는 밴드를 배경보다 한 단계 더 어둡게 둔다.

**개정 2026-09-01 — 라이트 밴드를 종이색으로 뒤집었다.** 원안은 라이트에서도 밴드를 `#0B1220` 짙은 네이비로 뒀는데, 흰 본문 위 검은 띠가 무거워 화면 첫인상을 지배했다(원래의 "밴드가 무거워 월간 문서 표지는 별도 헤더가 필요하다" 는 기록이 그 증상이었다). 라이트는 밴드를 `--surface` 와 같은 종이색으로 두고 글자를 잉크로 뒤집는다. 밴드와 본문(`#FFFFFF`)의 차이가 미세해지므로 헤더 아래 경계는 `--hairline` 으로 긋는다. **다크는 그대로다** — 밴드가 배경보다 어두운 관계가 유지된다.

밴드 위 알파 값(`band-foreground/70` 등 23곳)은 다시 잡지 않았다. 흰 글자 기준으로 고른 비율이 잉크 기준으로 뒤집혀도 같은 역할을 한다 — `/70` 은 라이트에서 `#4A505E` 로 `--muted-foreground` 근처다. 파이프라인 노드 배경 `band-foreground/[0.03]` 은 다크에서 도드라지고 라이트에서 파이는데, 방향은 반대지만 구분은 남으므로 그대로 뒀다(비활성 테두리만 `/35` → `/25`).
...
| `--band` | `#F5F6F8` | `#060A14` | 헤더·히어로 밴드 |
| `--band-foreground` | `#0B1220` | `#F2F4F8` | 밴드 위 글자 |
...
- **홈 밴드 개정 2026-09-01 — 도식의 글자를 절반으로 줄였다.** 노드는 제목·큰 수·**손이 가야 하는 것 한 줄**만 남긴다. 공급자 이름(`네이버 · 구글`·`Anthropic`)·분석 항목 나열·탈락 사유·결측 집계처럼 단계가 무슨 일을 하는지 설명하는 말은 지웠다 — 그걸 보고 정하는 것이 없다. 연결선의 규칙 캡션(`primary 기사만`·`judge`·`사업자번호`)도 지우고 선만 남긴다(실제로 `사업자번`/`호` 로 깨져 읽혔다). 산출 행은 요약문·우측 버튼·아래 사건 절과 전부 겹쳐 통째로 뺐다. 조각 30개 → 14개.
