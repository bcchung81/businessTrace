...
- **동명 검증 요청 품질 개선 반영 완료**: fsc 정규화 정확 일치만(케미타운 오탐 소멸) · DART 후보 SPC 필터(유동화·제N차/호) · 연금 번호 불일치는 후보 선택이 아니라 "미가입 가능성" absent 표기 · 해소된 source_conflict 사건 자동 done + 확인 필요 블록에서 사건 중복 노출 제거. 재조회 후 충돌 11→5건(써로마인드 DART 1 + 이름 완전 동일 fsc 4: 흥일기업·리브릿지·빅픽쳐스·에이치에이치에스), 오탐 사건 6건 자동 정리.

## 1. 첫 30분

- [ ] `/history` 2025 탭이 확정 48건으로 그려지는지, 재확정 필요하면 `/ranking` → 시상 확정.
- [ ] 사람 판단 UI 작업(자동화 불가, 오탐 정리 후 5건): 써로마인드(14) DART 후보 판정 · **금융위 번호 불일치 4건**(흥일기업·리브릿지·빅픽쳐스·에이치에이치에스 — 이름 완전 동일·번호 상이) 수용/배제 · 이퀄라이저(52)·그리너리(54) 검색 별칭 입력 후 재분석.
