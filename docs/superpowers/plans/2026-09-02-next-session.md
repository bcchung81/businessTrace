...
- 시험 3개사(2024 코호트: 크립토랩·올림플래닛·넷록스) 전체 파이프라인 verified 확인 후, **수정된 수집기로 56개사 전체 분석 배치를 백그라운드로 재실행해 둔 채 세션 종료** — 결과 미확인.

## 1. 첫 30분

- [ ] 전체 배치 결과 확인: 대시보드 밴드(분석 x/56, 검증 통과 수), "무보도" 가 줄었는지(수정 전 34개사), 레이트리밋으로 중단됐으면 남은 기업만 재실행. 세션 로그는 스크래치라 사라짐 — DB 기준으로 본다(`AnalysisRun` 최근, `/reports` 01).
