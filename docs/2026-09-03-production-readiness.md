...
## 7-a. 2026-09-04 — 성장 축 (제안 A1~A4·B1·B2·C6 반영)

랭킹의 `finance` 가 매출 절대값이라 큰 기업이 이기던 것을 **규모(finance)와 속도(growth)로 분리**했다. `formulaVersion rank-v1 → rank-v2`.

| 신호 | 원천 | 실측 커버리지(50개사) | 규칙 |
|---|---|---:|---|
| 매출 증가율 | DART `previous` | 12 | 전년 ≤0 이면 결측 |
| 고용 증감 | 국민연금 | 44 | 12개월 전 같은 달 우선, 없으면 ≥6개월 구간 연 환산 |
| 입·퇴사 순증 | 국민연금 hired/departed | 16 | 둘 다 보고된 달 ≥6개월만, 연 환산 |
| 조달 수주 추이 | 나라장터 `years[]` | **0** | 최근 2년 vs 직전 2년. **수집이 12개월치뿐이라 비어 있다** |

하위 신호는 각각 코호트 안에서 정규화한 뒤 평균한다. 고용 증감과 입·퇴사 순증은 **한 묶음**으로 평균해 고용 계열이 1/3 을 넘지 않게 했다 — 수상 기사 하나가 세 지표를 움직이던 것(§2 프롬프트 문제 1)과 같은 이중 계산을 피하기 위해서다. 못 잰 것은 전부 null 이고 가중치에서 빠진다.

결과: **성장 축이 45/50(90%)에 닿는다.** 재무 절대값은 12/50 이었다.

`profit_turn`(흑자 전환) 사건을 추가했다 — 전년 영업손실 → 당년 영업이익. 손실이 줄어든 것은 전환이 아니다. 실데이터 2건.

**2025 확정 48건이 rank-v1 이다.** 대시보드 추이 라벨이 `formulaDrift` 로 "산식이 바뀌어 변동에 계산식 변경이 섞여 있다" 를 적는다. 비교를 되살리려면 재확정이 필요하다.

조달 추이를 살리려면 `scripts/collect-procurement.ts` 를 48개월로 다시 돌려야 한다(12개월에 약 40분).

### 같은 날 이어서 — A5 · A6 · C3 · C4

- **A5 재무 축을 1인당으로.** `finance` = (매출 ?? 조달 수주액) ÷ 최신 국민연금 가입자. 인원을 모르면 결측이지 절대값으로 되돌아가지 않는다. 실측 커버리지 **12 → 27/50**(매출 12 · 조달 대리 15). ⚠️ 조달 수주액은 매출의 **하한**이라 대리지표 기업이 같은 칸에서 체계적으로 낮게 나온다 — 2026-08-28 문서 §4② 의 "가점 전용" 원칙과 어긋나는 절충이다. 별도 축으로 뺄지는 결과를 보고 정한다.
- **A6 감성 루브릭에서 수상·투자 제거.** "4~7: 성장, 수상, 투자 유치" 를 "성장, 신규 고객·시장 확보" 로 바꾸고 "수상·투자 사실 자체는 감성 점수에 반영하지 말라" 를 명시했다. 삼중 계산 해소. **다음 분석 실행부터** 반영된다 — 저장된 결과는 그대로다.
- **C3 확신도.** `about_confidence`(0~1)를 묻고 `ABOUT_CONFIDENCE_MIN = 0.7` 미만이면 "그 회사" 라고 답했어도 집계에서 뺀다. 없으면 1 로 본다(예전 결과 호환).
- **C4 부정 사건 종류.** `negative_kind` ∈ lawsuit·recall·sanction·none. 감성 ≤ −4 이면서 앞 셋이면 **경보**(`소송`·`리콜`·`규제 제재` 사건), 아니면 종전대로 `부정 보도` 주의. 없으면 none.

C3·C4 는 zod 선택 필드라 저장된 12개 픽스처·과거 실행 결과가 그대로 읽힌다.

### 같은 날 이어서 — B3 · C1 · C2 · C5 (산식 rank-v3)

- **B3 지속가능성 축(`stability`).** 재무 3비율 — 부채비율(낮을수록 좋음, 방향 반전)·ROE·영업이익률 — 을 각각 정규화한 뒤 관측된 것만 평균. 자본잠식이면 부채비율·ROE 는 결측이다(수치가 나와도 뜻이 뒤집힌다). 국세청 계속사업자·벤처확인은 이미 사건·감점으로 들어가 있어 이 축에 넣지 않았다(이중 계산). 커버리지는 DART 재무가 있는 곳뿐.
- **C5 투자 라운드·금액.** `investment_round`(seed·series_a·series_b·series_c_plus·ipo·other·none)와 `investment_amount_krw`(원, 없으면 null, "추정하지 말라"). 사건 제목 「투자 — 시리즈A 120억」, 검증 주장 「[주장 N-투자] 시리즈A · 120억원: …」 — **금액이 주장에 들어가 judge 가 기사와 대조한다.** 환각 금액은 불지지로 잡힌다.
- **C1 성장 신호.** `growth_signals: string[]` — 기사에 적힌 성장 사실을 기사 표현 그대로. 각 항목이 「[주장 N-성장] …」 로 문장별 판정을 받는다. **벤치마킹 지표로는 넣지 않았다** — 기사 속 자기 주장이 공공데이터 성장 축과 섞이면 안 된다.
- **C2 종합의견에 공공데이터 사실 주입.** `publicFactLines(CompanyFacts)` 가 설립·업력, 가입자·입퇴사, 매출·영업이익(+전년 대비), 조달 낙찰, 계속사업자, 벤처확인을 **코드가 계산한 한 줄 문장**으로 만든다. 종합의견 프롬프트에 「공식 원천 사실」 블록으로 넣고 "거기 있는 것만 인용" 을 지시하며, **같은 블록을 judge 에게도 넘겨** "공식 원천 사실을 인용한 부분은 기사에 없어도 supported" 규칙을 붙였다. 안 그러면 종합의견이 사실을 인용할수록 검증이 스스로를 벌한다. 사실이 없으면 블록을 아예 내지 않는다 — "없음" 은 화제가 된다.

원칙은 유지됐다: **공공데이터 숫자는 LLM 을 거치지 않는다.** LLM 이 뽑는 것은 기사 속 주장(라운드·금액·성장 문장)뿐이고 전부 검증 대상이다.
