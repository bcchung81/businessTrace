 ...
 * 사용: npx tsx scripts/monthly-report.ts YYYY-MM
 */
async function main() {
  const arg = process.argv[2];
  const match = arg?.match(/^(\d{4})-(\d{2})$/);
  if (!match) throw new Error("사용법: npx tsx scripts/monthly-report.ts YYYY-MM");

  const year = Number(match[1]);
  const month = Number(match[2]);

  const { events, cards, freshness } = await loadMonthlyReportInput(year, month);
  ...
  console.log(`[OK] ${filePath} (${events.length}건 사건 · ${cards.length}개사)`);
}
