...
function rows(): RankingRow[] {
  const ranked = rankCompanies(
    [
      { companyId: 1, name: "㈜가", industry: "SW", sentiment: 8, awards: 1, investments: 1, revenue: 100, verification: "verified", growth: { revenue: null, headcount: null, hiring: null, procurement: null }, stability: { debtRatio: null, roe: null, operatingMargin: null }, confirmedRisks: 0 },
      { companyId: 2, name: "㈜나", industry: "의료/헬스케어", sentiment: 2, awards: 0, investments: 0, revenue: null, verification: "needs_review", growth: { revenue: null, headcount: null, hiring: null, procurement: null }, stability: { debtRatio: null, roe: null, operatingMargin: null }, confirmedRisks: 1 },
      { companyId: 3, name: "㈜다", industry: null, sentiment: null, awards: null, investments: null, revenue: null, verification: null, growth: { revenue: null, headcount: null, hiring: null, procurement: null }, stability: { debtRatio: null, roe: null, operatingMargin: null }, confirmedRisks: 0 },
    ],
    loadRubrics(),
  );
...
describe("RankingTable", () => {
  test("pages twenty rows at a time", () => {
    const many = rankCompanies(
      Array.from({ length: 25 }, (_, index) => ({
        ...
        sentiment: index, awards: 0, investments: 0, revenue: null, verification: "verified" as const, growth: { revenue: null, headcount: null, hiring: null, procurement: null }, stability: { debtRatio: null, roe: null, operatingMargin: null }, confirmedRisks: 0,
      })),
      loadRubrics(),
  ...
  test("shows the thirteen columns in the brief's order", () => {
    render(<RankingTable rows={rows()} industries={["SW", "의료/헬스케어"]} />);
    const headers = screen.getAllByRole("columnheader").map((th) => th.textContent);
    expect(headers).toEqual(["순위", "기업", "판정", "총점", "감성", "수상", "투자", "재무", "성장", "안정", "검증", "리스크 감점", "산업"]);
  });
  ...
  test("prints the penalty as a negative number only when a risk was confirmed", () => {
    render(<RankingTable rows={rows()} industries={[]} />);
    expect(within(screen.getByRole("row", { name: /㈜나/ })).getAllByRole("cell")[11]).toHaveTextContent("-0.20");
    expect(within(screen.getByRole("row", { name: /㈜가/ })).getAllByRole("cell")[11]).toHaveTextContent("0");
  });
