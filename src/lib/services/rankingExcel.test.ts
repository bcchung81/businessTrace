...
describe("ranking workbook", () => {
  test("writes the weight line, one row per company and — for missing metrics", async () => {
    ...
    const rows = rankCompanies(
      [
        { companyId: 1, name: "㈜가", industry: "SW", sentiment: 5, awards: 1, investments: 0, revenue: null, verification: "verified", growth: { revenue: null, headcount: null, hiring: null, procurement: null }, stability: { debtRatio: null, roe: null, operatingMargin: null }, confirmedRisks: 0 },
        { companyId: 2, name: "㈜나", industry: null, sentiment: null, awards: null, investments: null, revenue: null, verification: null, growth: { revenue: null, headcount: null, hiring: null, procurement: null }, stability: { debtRatio: null, roe: null, operatingMargin: null }, confirmedRisks: 0 },
      ],
      book,
    );
    ...
    expect(String(sheet.getCell("A2").value)).toContain("감성 0.22 · 수상 0.13");
    expect(sheet.getRow(4).values).toEqual([undefined, "순위", "기업", "판정", "총점", "감성", "수상", "투자", "재무", "성장", "안정", "검증", "리스크 감점", "산업", "루브릭"]);
  ...
  test("prints the verdict the screen shows — a namesake conflict outranks the judge", async () => {
    ...
    const rows = rankCompanies(
      [
        { companyId: 1, name: "㈜충돌", industry: "SW", sentiment: 5, awards: 1, investments: 0, revenue: null, verification: "verified", growth: { revenue: null, headcount: null, hiring: null, procurement: null }, stability: { debtRatio: null, roe: null, operatingMargin: null }, confirmedRisks: 0 },
        { companyId: 2, name: "㈜통과", industry: "SW", sentiment: 9, awards: 3, investments: 1, revenue: null, verification: "verified", growth: { revenue: null, headcount: null, hiring: null, procurement: null }, stability: { debtRatio: null, roe: null, operatingMargin: null }, confirmedRisks: 0 },
      ],
      book,
    );
