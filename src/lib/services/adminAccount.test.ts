...
describe("createAdminAccount", () => {
  ...
  it("creates an account the login flow can then authenticate", async () => {
    const result = await createAdminAccount({ email: "admin@kca.kr", password: "Passw0rd!" });
    ...
    expect(await verifyPassword("Passw0rd!", stored.passwordHash)).toBe(true);
    expect(stored.isActive).toBe(true);
  });

  it("normalises the email the same way the login flow does", async () => {
    await createAdminAccount({ email: "  Admin@KCA.kr  ", password: "Passw0rd!" });
  ...
  it("refuses a duplicate email instead of overwriting an existing account", async () => {
    ...
    const result = await createAdminAccount({ email: "admin@kca.kr", password: "Different1!" });
  ...
  it("refuses a weak password and creates nothing", async () => {
    ...
    expect(result).toEqual({ ok: false, message: "비밀번호는 최소 8자 이상이어야 합니다." });
    expect(await prisma.user.count()).toBe(0);
  });

  it("refuses a malformed email and creates nothing", async () => {
    const result = await createAdminAccount({ email: "not-an-email", password: "Passw0rd!" });
