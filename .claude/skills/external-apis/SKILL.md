...
**재무 결측이 주 경로다.** 검증 대상 5개사 중 4개사는 DART 고유번호조차 없다. 벤치마킹의 결측 지표 제외 정규화는 예외 처리가 아니라 **기본 동작**이다.

**`fnlttSinglAcnt` 는 정기보고서 제출 법인만 수록한다.** 감사보고서만 내는 외감 비상장사는 원문 공시에 재무제표가 있어도 `013 조회된 데이타가 없습니다` 다 — 이것을 "비외감" 으로 읽으면 틀린다. `list.json` 에서 `감사보고서 (YYYY.12)`(연결 제외)를 찾아 `document.xml`(zip) 을 풀면 재무제표가 나온다 (`dartAuditReport.ts`). 실측 50개사 중 10개사가 이 경로로만 열렸다.

- 원문 마크업은 두 규격이다 — `ACLASS="FINANCE"` 표의 `ADELIM` 셀, 그리고 ADELIM 없는 일반 `TD` 표. 후자에서 실제로 틀렸던 것 셋: 둘째 셀이 주석 참조 번호라 **마지막 두 숫자 셀**을 당기·전기로 읽어야 하고, 목차·주석 표가 계정을 한 번 스치므로 **대상 계정 2종 이상인 표만** 봐야 하며, `영업이익` 행에 담긴 괄호 음수를 `abs` 로 지우면 안 된다
- 과목명은 `자      산      총      계` 처럼 공백이 박히고 `(주23)` 이 붙는다. 단위는 `AUNITVALUE` 또는 본문 `(단위 : 천원)`
- `fnlttXbrl` 은 XBRL 미제출이면 `014` 다 — 폴백으로 쓸 수 없다
