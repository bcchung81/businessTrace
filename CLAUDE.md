...
기존 Flask 앱을 **Next.js 풀스택**으로 재구축하는 중이다. **관리자 전용 도구다** — 공개 회원가입을 열지 않고 계정은 `npx tsx scripts/create-admin.ts` 로 발급한다. 자율 가입은 향후 확장 사항이다. 레거시 데이터는 이관하지 않고 신규 시스템으로 새로 만든다. Phase 0 은 Task 1·2a·2b·2c·2d 까지 완료됐고 **핵심 루프(7 수집 → 8 분석 → 9 검증 → 10 리포트)가 완료됐다.** Phase A(4 국세청·5 DART·5b 나라장터)도 완료됐고, Task 5 잔여(동명 충돌 후보 선택·사업자번호 입력)는 기업 상세 "확인 필요" 블록으로 완료됐다. 분석 실행 UI(일괄 실행 화면 `/companies` + 배치 스크립트)와 사건 모니터링(대시보드·기업 카드·월간 문서)은 완료됐다. Phase B 도 완료됐다(12 벤치마킹 랭킹·13 기여도/인용·14 이력/시상, 11 은 사건 모니터링이 대체). 배포(16)는 **서버에서 Node 를 직접 돌리는 구성**이다(`deploy/` — systemd 유닛·nginx·배포/백업 스크립트). 도커는 쓰지 않는다. 서버 실기동과 PostgreSQL 전환이 남았다. Task 6(재무 비율)은 dartlab 없이 자체 계산으로 닫았고 **딥리서치(15·15b)는 폐기했다.** **사용자 편의성 개선(2026-09-02 플랜, 15개 태스크)도 완료됐다** — 기업 편집·제외, 배치 서버 완주·재접속, 로딩·오류·404, 기업명 검색, URL 표 상태, 로그아웃 등. **프로덕션 완성 개선안(`docs/2026-09-03-production-readiness.md`)의 §1 배포 차단·§2 배포 전 필수도 전부 처리했다** — SSRF·레이트리밋·프롬프트 인젝션·세션·보안헤더·라우트 그룹 인가, 구조화 로그·LLM 침묵·백업·환경변수 게이트, 일괄 검토 근거·큐 걷기·배치 붕괴·「미확인」 정의 통일. **랭킹에 성장 축(`growth`: 매출 YoY·고용 증감·입퇴사 순증·조달 추이, 산식 `rank-v2`)을 더했다** — 규모(`finance`)와 속도를 분리한 것이고, 하위 신호는 각각 정규화한 뒤 고용 계열을 한 묶음으로 평균한다. 못 잰 신호는 null 로 가중치에서 빠진다. 재무(`finance`)는 1인당(매출 ?? 조달 수주액 ÷ 가입자)이고, 감성 루브릭은 수상·투자를 세지 않으며, 동향 분석은 `about_confidence`(문턱 0.7)·`negative_kind`(소송·리콜·제재 → 경보)·`growth_signals`(기사 속 성장 문장 → 검증 주장)를, 투자 분석은 라운드·금액(원)을 함께 받는다. 지속가능성 축(`stability`: 부채비율 반전·ROE·영업이익률)이 있고 산식은 `rank-v3` 다. 종합의견에는 `publicFactLines` 가 만든 공식 원천 사실 한 줄들이 들어가고 **같은 줄을 judge 에게도 넘긴다** — 공공데이터 숫자는 LLM 을 거치지 않는다. 남은 것은 그 문서의 §3(정리)·§4(PostgreSQL)와 서버 실기동, 조달 48개월 재수집이다.
