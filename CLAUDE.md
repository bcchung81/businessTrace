...
기존 Flask 앱을 **Next.js 풀스택 + Python 사이드카**로 재구축하는 중이다. **관리자 전용 도구다** — 공개 회원가입을 열지 않고 계정은 `npx tsx scripts/create-admin.ts` 로 발급한다. 자율 가입은 향후 확장 사항이다. 레거시 데이터는 이관하지 않고 신규 시스템으로 새로 만든다. Phase 0 은 Task 1·2a·2b·2c·2d 까지 완료됐고 **핵심 루프(7 수집 → 8 분석 → 9 검증 → 10 리포트)가 완료됐다.** Phase A(4 국세청·5 DART·5b 나라장터)도 완료됐고, Task 5 잔여(동명 충돌 후보 선택·사업자번호 입력)는 기업 상세 "확인 필요" 블록으로 완료됐다. 분석 실행 UI(일괄 실행 화면 `/companies` + 배치 스크립트)와 사건 모니터링(대시보드·기업 카드·월간 문서)은 완료됐다. 남은 것은 사이드카(3·6·15), Phase B 중 14(12 벤치마킹 랭킹·13 기여도/인용 근거는 완료, 11 은 사건 모니터링이 대체), 배포(16)다.
